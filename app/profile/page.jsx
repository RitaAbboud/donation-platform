...
const deleteRequest = async (id) => {
  ...
  if (loading) {
    return (
      <div className="min-h-screen flex items-center justify-center text-sm text-slate-500 bg-[#fff7f0]">
        Loading profile…
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-[#fff7f0]">
      ...
      <div className="max-w-6xl mx-auto px-6 pt-32 pb-32 space-y-20">
        ...
        <div>
          <div className="flex items-center justify-between mb-10">
            <h2 className="text-2xl font-extrabold tracking-tight text-[#e25e2d]">Bundle Requests</h2>
            <button
...
function EditModal({ editing, editForm, setEditForm, handleUpdate, setEditing, categories, loading }) {
  return (
    <>
      <div className="fixed inset-0 bg-slate-900/30 backdrop-blur-sm z-[100]" onClick={() => setEditing(null)} />
      <div className="fixed right-0 top-0 h-full w-full max-w-xl bg-[#fff6ef] shadow-2xl z-[110] overflow-y-auto p-8 animate-in slide-in-from-right duration-300">
        <div className="flex justify-between items-center mb-8">
          ...
          <button onClick={() => setEditing(null)} className="text-slate-400 hover:text-red-500 text-2xl font-bold">×</button>
        </div>
        ...
        <div className="space-y-6 bg-white p-6 rounded-2xl border border-[#fae9d7]">
          ...
          <div className="flex flex-col gap-2">
            ...
            <select value={editForm.category_id} onChange={(e) => setEditForm({ ...editForm, category_id: e.target.value })} className="w-full rounded-xl border border-[#fae9d7] bg-[#fff6ef] px-4 py-3 text-sm focus:border-[#e25e2d] outline-none transition-all">
          ...
          <div className="flex flex-col gap-2">
            ...
            <textarea rows={3} value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} placeholder="Describe..." className="rounded-xl border border-[#fae9d7] bg-[#fff6ef] px-4 py-3 text-sm focus:border-[#e25e2d] outline-none transition-all" />
          </div>

          {/* Location */}
          <div className="flex flex-col gap-2">
            <label className="text-xs font-black uppercase tracking-widest text-slate-400 ml-1">Location</label>
            <input type="text" value={editForm.location} onChange={(e) => setEditForm({ ...editForm, location: e.target.value })} placeholder="Enter street or neighborhood" className="w-full rounded-xl border border-[#fae9d7] bg-[#fff6ef] px-4 py-3 text-sm focus:border-[#e25e2d] outline-none transition-all" />
          </div>
          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative">
              ...
              <input type="text" value={editForm.phone} onChange={(e) => setEditForm({ ...editForm, phone: e.target.value })} placeholder="71 234 567" className="w-full rounded-xl border border-[#fae9d7] bg-[#fff6ef] pl-10 pr-4 py-3 text-sm focus:border-[#e25e2d] outline-none transition-all" />
            </div>
          </div>

          <div className="pt-6 flex gap-4">
            <button onClick={() => setEditing(null)} className="flex-1 px-6 py-3 rounded-xl font-bold text-slate-400 hover:bg-slate-50 transition-colors">Cancel</button>
            <button onClick={() => handleUpdate(editing.id)} disabled={loading} className="flex-1 bg-[#e25e2d] text-white px-6 py-3 rounded-xl font-bold hover:bg-[#d14d1c] transition-all shadow-lg shadow-orange-200 active:scale-[0.98] disabled:opacity-50">
